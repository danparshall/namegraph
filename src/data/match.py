...
def match_by_cedula_padre(names_padre):
    ...
    names_padre = names_padre[names_padre['ced_padre']!='']
    return names_padre
    

def match_by_cedula_madre(names_madre):
    names_madre = names_madre[["cedula", "ced_madre"]].copy()
    names_madre = names_madre[names_madre['ced_madre'] != '']
    return names_madre
...
def exact_name_padre(ncleaned, ceds_found):
    ...
    """
    ncleaned_rf = ncleaned[~ncleaned.cedula.isin(ceds_found)]
    ...
    p1 = valid_matched_padres[['cedula', 'ced_pad']
                              ].rename(columns={'ced_pad': 'padre_matched'})
    p2 = alternate_matched_padres[['cedula', 'ced_pad']
                                ].rename(columns={'ced_pad': 'padre_matched'})
...
def exact_name_madre(ncleaned, ceds_found):
    ...
    m1 = valid_matched_madres[['cedula', 'ced_mad']
                              ].rename(columns={'ced_mad': 'madre_matched'})
    m2 = alternate_matched_madres[['cedula', 'ced_mad']
                                  ].rename(columns={'ced_mad': 'madre_matched'})
...
def ceds_found(names, ced_parent = None, matched = None, only_cedula = False):
    """ People with padre or madre cedula or exact name matched

    Args:
        names       : Clean dataframe generated by create_names()
        ced_parent  : name of column 'ced_madre' or 'ced_padre' 
        matched     : Exact name matches generated by exact_name()
        only_cedula : Only interested in cedulas with padre or madre cedula, not matching
    ...
    if only_cedula:
        ...
        ceds_found = set(names[ced].cedula)
    else:
